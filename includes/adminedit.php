<?php
	...
	if (isset($_POST['edit'])) {
		...
		if (empty($error)) {
			...
			$sql = "UPDATE `admin` SET `email` = '$email', `username` = '$username' WHERE `id` = '$id' AND deleted = 1";
			$result = mysqli_query($connect, $sql);

			if ($result) {
				$success = "user details Updated Sucessfully";
				header("location: ../admin/adminprofile.php?success=".$success);
			}else{
				$failed = "Error Updating user details";
				header("location: ../admin/adminprofile.php?error=".$failed);
			}
		}else{
			header('location: ../admin/admin.php?error='.join($error, urlencode('<br>')));
		}
	}
	...
	if (isset($_POST['edit2'])) {
		...
		if (empty($error)) {
			...
			$sql = "UPDATE `admin` SET `password` = '$new_password' WHERE `id` = '$id' AND deleted = 1";
			$result = mysqli_query($connect, $sql);
			...
			header('location: ../admin/adminprofile.php?error='.join($error, urlencode('<br>')));
		}
	}
		      ...
		      if(in_array($filetype, $allowtypes)){
		        ...
		        if ($imagesize < 400000) {
		          ...
		          if($compressedimage){
		            ...
		            $sql = "UPDATE `admin` SET `photo` = '$pic' WHERE `id` = '$id' AND deleted = 1";
		            $result = mysqli_query($connect, $sql);
